...
__webpack_require__.d(__webpack_exports__, {
  DynamicPrice: () => (/* binding */ DynamicPrice)
});
...
;// CONCATENATED MODULE: ./src/components/dynamic-price/dynamic-price-styles.ts

const dynamicPriceStyles = i `
  :host {
...
;// CONCATENATED MODULE: ./src/components/dynamic-price/index.ts
...
(function (DisplayType) {
    DisplayType["price"] = "price";
    DisplayType["labeledPrice"] = "labeled-price";
    DisplayType["savings"] = "savings";
})(DisplayType || (DisplayType = {}));
let DynamicPrice = class DynamicPrice extends I18nMixin(lit_element_s) {
    constructor() {
        super(...arguments);
        this.pid = '';
        this.sku = '';
        ...
        this.locale = 'en-us';
        this.env = 'prod';
        this.displayType = DisplayType.price;
        this.errorText = '(Price unavailable)';
        this.productTask = new task_h(this, {
            task: async ([pid, isM365, locale, env], { signal }) => {
                if (!pid)
                ...
                env =
                    env ||
                        (location.hostname.includes('www.') ||
                            location.hostname.includes('adobeprod')
                            ? 'prod'
                            : 'ppe');
                const response = await fetch(`https://www${env === 'prod' ? '' : 'ppe'}.microsoft.com/msstoreapi${env === 'prod' ? 'prod' : 'ppe'}/api/buyBox` +
                    `?locale=${locale}` +
                    `&productId=${pid}` +
                    `&environment=${env === 'prod' ? 'prod' : 'ppe'}` +
                    `${isM365 ? '&isM365Page=true' : ''}`, { signal });
                /* istanbul ignore if */
                if (!response)
                    throw new Error(`DynamicPrice fetch no response`);
                /* istanbul ignore if */
                if (!response.ok)
                    throw new Error(`DynamicPrice fetch bad response: ${response}`);
                const data = response.json();
                /* istanbul ignore if */
                if (!data)
                    throw new Error(`DynamicPrice fetch: data is empty`);
                return data;
            },
            args: () => [
                this.pid,
        ...
        this.extract = (data) => {
            ...
            switch (this.displayType) {
                ...
                case DisplayType.labeledPrice:
                    return this.extractPrice(product);
                case DisplayType.savings:
                    return this.extractSavings(product);
                default:
                    throw new Error(`(Display type ${this.displayType} not supported)`);
            }
        };
        this.extractSavings = (product) => {
            const savingsObj = this.sku
                ? (product.skuInfo?.[this.sku.toUpperCase()] ||
                    product.skuInfo?.[this.sku.toLowerCase()])?.badge
                : Object.values(product.skuInfo || {})[0]?.badge;
            if (!savingsObj)
                throw new Error('(Savings not found)');
            if (savingsObj.badgeFormat) {
                // Handles any localized text that does not follow the "label savings" format
                return (this.format(savingsObj.badgeFormat, { '0': savingsObj.savingsPrice }) ||
                    this.errorText);
            }
            else {
                return (
                // Fallback for no badgeFormat provided ( Buy Box returns no badgeFormat when expecting a label-savings format )
                x ` ${savingsObj.savingsType}
        ${savingsObj.savingsPrice || this.errorText}`);
            }
        };
        this.extractPrice = (product) => {
            const priceObj = this.sku
                ? (product.skuInfo?.[this.sku.toUpperCase()] ||
                    product.skuInfo?.[this.sku.toLowerCase()])?.price
                : Object.values(product.skuInfo || {})
                    .map((v) => v.price)
                    .reduce((a, v) => (a && a.currentValue < v?.currentValue ? a : v), undefined);
            if (!priceObj)
                throw new Error('(Price not found)');
            const currentDisplayPrice = priceObj.recurrencePrice || priceObj.currentPrice;
            if (this.displayType === DisplayType.labeledPrice) {
                const price = priceObj.originalPrice
                    ? x ` <span part="original-price" class="original-price"
              >${priceObj.originalPrice}</span
            >
            <span part="discounted-price" class="discounted-price"
              >${currentDisplayPrice}</span
            >`
                    : currentDisplayPrice;
                if (priceObj.priceFormat) {
                    return (
                    // Handles any localized text that does not follow the "label price" format
                    this.format(priceObj.priceFormat, { '0': price }) || this.errorText);
                }
                else {
                    // Fallback for no priceFormat provided ( Buy Box returns no priceFormat when expecting a label-price format )
                    return x ` ${priceObj.fromText} ${price}` || this.errorText;
                }
            }
            else {
                return currentDisplayPrice || this.errorText;
            }
        };
    }
    updated() {
        if (this.textContent !== this.shadowRoot?.textContent) {
            const textContent = (this.textContent =
                this.shadowRoot?.textContent || '');
            this.dispatchEvent(new CustomEvent('DYNAMIC-PRICE-UPDATED', {
                bubbles: true,
                composed: true,
                detail: { priceText: textContent },
            }));
        }
    }
    render() {
        return this.productTask.render({
            pending: () => x `<span class="ellipsis-loading"
        ...
        >`,
            complete: (data) => {
                ...
                try {
                    ...
                    display = `${this.errorText}`;
                }
                return x `${display}`;
            },
            error: (error) => {
                console.log(error);
                return x `${this.errorText}`;
            },
        });
    }
    firstUpdated() {
...
DynamicPrice.styles = dynamicPriceStyles;
__decorate([
    property_n({ attribute: 'pid' })
], DynamicPrice.prototype, "pid", void 0);
__decorate([
    property_n({ attribute: 'sku' })
], DynamicPrice.prototype, "sku", void 0);
__decorate([
    property_n({ attribute: 'ism365', type: Boolean })
], DynamicPrice.prototype, "isM365", void 0);
__decorate([
    property_n({ attribute: 'locale' })
], DynamicPrice.prototype, "locale", void 0);
__decorate([
    property_n({ attribute: 'env' })
], DynamicPrice.prototype, "env", void 0);
__decorate([
    property_n({ attribute: 'display-type' })
], DynamicPrice.prototype, "displayType", void 0);
__decorate([
    property_n({ attribute: 'error-text' })
], DynamicPrice.prototype, "errorText", void 0);
DynamicPrice = __decorate([
    defineCustomElement('dynamic-price')
], DynamicPrice);
