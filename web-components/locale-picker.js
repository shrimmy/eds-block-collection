...
__webpack_require__.d(__webpack_exports__, {
  LocalePicker: () => (/* binding */ LocalePicker)
});
...
;// CONCATENATED MODULE: ./src/components/locale-picker/locale-picker.styles.ts

/* harmony default export */ const locale_picker_styles = (i `
  .grid-container {
    display: grid;
...
;// CONCATENATED MODULE: ./src/components/locale-picker/index.ts
var LocalePicker_1;




let LocalePicker = LocalePicker_1 = class LocalePicker extends lit_element_s {
    constructor() {
        super(...arguments);
        this.config = null;
        this._columns = 4;
    ...
    renderLocaleList(locale) {
        return x `
      <li>
        <a href=${this.getUrl(locale.locale, locale.overrideUrl)}
          >${locale.nativeName}</a
        >
      </li>
    `;
    }
    renderWithoutGrouping() {
        const locales = [...(this.config.regions[0].locales || [])];
        const localesPerColumn = Math.ceil(locales.length / this.columns);
        return x `
      ${Array.from({ length: this.columns }, (_, i) => x `
          <ul>
            ${locales
            .slice(i * localesPerColumn, (i + 1) * localesPerColumn)
            .map((locale) => this.renderLocaleList(locale))}
          </ul>
        `)}
    `;
    }
    render() {
        if (!this.config ||
            ...
            console.warn('Locale Picker configuration is not valid.');
            return x ``;
        }
        return this.config.hasRegions
            ? x `
          <div class="grid-container">
            ${this.config.regions?.map((region) => x `
                <ul>
                  <li>
                    <p>${region.regionName}</p>
                  </li>
                  ${region.locales?.map((locale) => this.renderLocaleList(locale))}
                </ul>
              `)}
          </div>
        `
    ...
    getUrl(locale, overrideUrl) {
        if (overrideUrl) {
            return overrideUrl;
        }
        let url = new URL(this.config.defaultUrl);
        if (document.referrer) {
            const referrer = new URL(document.referrer);
            if (LocalePicker_1.validHosts.includes(referrer.hostname)) {
                url = referrer;
            }
        }
        const urlWithSource = this.appendSourceQsp(url);
        return this.replaceLocale(urlWithSource, locale);
    }
    appendSourceQsp(url) {
        if (url.searchParams.has('source')) {
            return url;
        }
        url.searchParams.append('source', 'lp');
        return url;
    }
    replaceLocale(url, locale) {
        const pathSegments = url.pathname.split('/');
        const languageIndex = pathSegments.findIndex((segment) => LocalePicker_1.localeRegEx.test(segment));
        if (languageIndex !== -1) {
            pathSegments[languageIndex] = locale;
        }
        url.pathname = pathSegments.join('/').toString();
        return url.toString();
    }
};
LocalePicker.styles = locale_picker_styles;
LocalePicker.localeRegEx = /^(\w{2,3}-\w{2,4}|\w{2,3}|\w{2,3}-\w{2,4}-\w{2,8})$/;
LocalePicker.validHosts = [
    'www.microsoft.com',
    'wwwqa.microsoft.com',
    'wwwppe.microsoft.com',
    'wwwdev.microsoft.com',
];
__decorate([
    property_n({ type: Object })
], LocalePicker.prototype, "config", void 0);
__decorate([
    property_n({ type: Number })
], LocalePicker.prototype, "columns", null);
LocalePicker = LocalePicker_1 = __decorate([
    custom_element_t('locale-picker')
], LocalePicker);
