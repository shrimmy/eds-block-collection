...
;// CONCATENATED MODULE: ./src/util/telemetry.ts
...
class Telemetry extends lit_element_s {
    constructor() {
        super(...arguments);
        this.options = {};
    }
    connectedCallback() {
        ...
        this.addEventListener(this.options.eventType || EventType.Click, this.handleEvent);
    }
    disconnectedCallback() {
        super.disconnectedCallback();
        this.removeEventListener(this.options.eventType || EventType.Click, this.handleEvent);
    }
    handleEvent(event) {
        // Find the element with the 'data-capture-telemetry' attribute in the event path
        const elementWithAttribute = event
            .composedPath()
            .find((element) => element instanceof HTMLElement &&
            element.hasAttribute('data-capture-telemetry'));
        if (elementWithAttribute) {
            try {
                // Extract telemetry data attributes from the element's 'data-capture-telemetry' attribute
                let dataAttributes;
                if (elementWithAttribute.hasAttribute('data-m')) {
                    dataAttributes = JSON.parse(elementWithAttribute.getAttribute('data-m') || '{}');
                }
                else {
                    const biAttributes = Array.from(elementWithAttribute.attributes).filter((attr) => attr.name.startsWith('data-bi'));
                    dataAttributes = biAttributes.reduce((tags, attr) => {
                        tags[attr.name.slice(8)] = attr.value;
                        return tags;
                    }, {});
                }
                // Dispatch telemetry capture event with captured element and tags
                //1DS guide -> https://microsoft.sharepoint.com/teams/MarTechPortal/SitePages/website-tools/oneds/guided-learning/overview.aspx
                window?.telemetry?.webAnalyticsPlugin?.capturePageAction?.(null, {
                    behavior: elementWithAttribute,
                    actionType: 'CL',
                ...
                console.error('Error capturing telemetry or dispatching event:', error);
            }
        }
    }
}
...
], Telemetry.prototype, "options", void 0);
// Utility function to generate a JSON string of telemetry tags
const get1DStags = ({ cN, ecn, id, cT, pa, hn, ehn, bhvr, assetid, compnm, uri, } = {}) => {
    const requiredTags = {
        cN: cN,
        ecn: ecn || cN,
        cT: cT,
    ...
    // Build the optional tag objects based on provided values
    const idTag = id === undefined ? {} : { id };
    const hnTag = hn === undefined ? {} : { hn };
    const ehnTag = ehn === undefined ? {} : { ehn: hn };
    const bhvrTag = bhvr === undefined ? {} : { bhvr };
    const assetidTag = assetid === undefined ? {} : { assetid };
    const uriTag = uri === undefined ? {} : { uri };
    // Combine required and optional tags into a single object and return JSON string
...
let PageNav = class PageNav extends I18nMixin(Telemetry) {
    constructor() {
        super(...arguments);
        /**
    ...
    render() {
      ...
      >
        <ul>
          ...
          ${isMobileView ? this._generateTabbableListItem() : ''}
        </ul>
      </div>
    `;
        return x `
