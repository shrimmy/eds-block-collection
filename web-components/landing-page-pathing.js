...
__webpack_require__.d(__webpack_exports__, {
  LandingPagePathing: () => (/* binding */ LandingPagePathing)
});
...
;// CONCATENATED MODULE: ./src/components/landing-page-pathing/landing-page-pathing.styles.ts

/* harmony default export */ const landing_page_pathing_styles = (i `
  .landing-page-path .lpp-article-content .lpp-image {
    overflow: hidden;
    border-radius: 16px;
  }
  .landing-page-path[article-count='3']
    article:nth-child(2)
    .lpp-image
    ::slotted(img),
  .landing-page-path[article-count='4']
    article:nth-child(2)
    .lpp-image
    ::slotted(img) {
    animation: 1s scale-up;
  }
  ...
  .landing-page-path .lpp-article-content > div {
    text-align: center;
    width: 100%;
    padding-bottom: 24px;
  }

  .landing-page-path[article-count='3'] .lpp-article-content .lpp-image {
    padding-bottom: 24px;
  }
  ...
  .landing-page-path .lpp-main article {
    position: relative;
  }

  .landing-page-path .content-flow {
    overflow-y: hidden;
  }
  ...
  @media (min-width: 540px) {
    ...
    .landing-page-path .lpp-main article.article-0 .lpp-image ::slotted(img),
    .landing-page-path[article-count='3']
      .lpp-main
      article.article-2
      .lpp-image
      ::slotted(img),
    .landing-page-path .lpp-main article.article-3 .lpp-image ::slotted(img) {
      max-height: 426px;
      max-width: 426px;
    }

    .landing-page-path .lpp-main article.article-1 .lpp-image ::slotted(img),
    .landing-page-path[article-count='3']
      .lpp-main
      article.article-1
      .lpp-image
    ...
    .landing-page-path .lpp-main article.article-0 .bottom-content,
    .landing-page-path[article-count='3']
      .lpp-main
      article.article-2
      .bottom-content,
    .landing-page-path .lpp-main article.article-3 .bottom-content {
      max-width: 426px;
    }

    .landing-page-path .lpp-main article.article-1 .bottom-content,
    .landing-page-path[article-count='3']
      .lpp-main
      article.article-1
      .bottom-content,
  ...
  @media (min-width: 1084px) {
    ...
    .landing-page-path[article-count='4'] .lpp-main {
      flex-direction: column;
    }

    .landing-page-path:not([article-count='3']) .lpp-main > div {
      margin: 0 auto;
      flex: 1 1 49%;
      flex-direction: row;
      justify-content: center;
    ...
    .landing-page-path:not([article-count='3']) .lpp-article-content {
      align-self: stretch;
      flex-direction: column;
      justify-content: flex-start;
    ...
    .landing-page-path:not([article-count='3']) .lpp-main article {
      display: flex;
    }

    .landing-page-path[article-count='3'] .lpp-main > div {
      flex-direction: column;
      justify-content: center;
      align-items: flex-start;
      gap: 24px;
...
;// CONCATENATED MODULE: ./src/components/landing-page-pathing/index.ts
...
let LandingPagePathing = class LandingPagePathing extends lit_element_s {
    constructor() {
        super(...arguments);
        /**
         * The total number of articles to display in the landing page pathing component.
         *
         * @remarks
    ...
    render() {
        return static_n `
      <section class="landing-page-path" article-count=${this.total}>
        <div class="container">
          <div class="row">
            <div class="lpp-content">
              <slot name="lpp-heading"></slot>
            </div>
            <div class="lpp-main">
              ${this.total == 3
            ? static_n `
                    <div>${this.renderArticle(0)}</div>
                    <div>${this.renderArticle(1)} ${this.renderArticle(2)}</div>
                  `
            : this.total > 3
                ? Array.from({ length: 2 }, (_, i) => i).map((i) => static_n `
                      <div>
                        ${Array.from({ length: Math.min(2, this.total - i * 2) }, (_, j) => j).map((j) => this.renderArticle(i * 2 + j))}
                      </div>
                    `)
                : static_n `
                    <div>
                      ${Array.from({ length: this.total }, (_, i) => i).map((i) => this.renderArticle(i))}
                    </div>
                  `}
            </div>
          </div>
    ...
    renderArticle(i) {
        return static_n `
      <article>
        <div class="lpp-article-content">
          <div class="lpp-image">
            <slot name="lpp-image${i + 1}"></slot>
          </div>
          <div class="bottom-content">
            <slot name="lpp-card${i + 1}"></slot>
          </div>
        </div>
      </article>
    `;
    ...
    firstUpdated() {
        for (let i = 0; i < this._lppArticles.length; i++) {
            this._lppArticles[i].classList.add('article-' + i);
        }
        if (this._headingSlot &&
            this._headingSlot.length > 0 &&
            this._headingSlot[0]) {
            this._heading = this._headingSlot[0].querySelector('[slot="title"]');
            this._description = this._headingSlot[0].querySelector('[slot="paragraph"]');
            this._topBtnGroup = this._headingSlot[0].querySelector('[slot="links"]');
        }
    }
    updated() {
        this.handleScrollEvent();
    }
    animateArticle(lpp, article, image, delay, opacity = 0, yAxis = 200) {
        if (article != null) {
            lpp.fromTo(article, { opacity: opacity, y: yAxis }, { opacity: 1, ease: 'power4.inout', y: 0, duration: 2 }, delay);
            if (image != null) {
                lpp.fromTo(image, { scale: 1.2 }, { scale: 1 }, '<+0.8');
            }
        }
    }
    /**
     * @function killScrollTriggerAnimation
     * This function is used to stop all ScrollTrigger animations and clear their properties.
     *
     * It iterates over all ScrollTriggers, kills each trigger and gets its associated animation. Killing the trigger will help to prevent further triggerin, free up memory resources
     * and rest state if we want to  cretea a new animation with a new ScrollTrigger.
     *
     * If the animation is an instance of gsap.core.Timeline, it gets all the child tweens and timelines of the animation and pushes their targets to the targets array.
     * If the animation is an instance of gsap.core.Tween, it pushes its targets to the targets array.
     *
     * After iterating over all triggers, it uses gsap.set() to clear all properties of the targets.
     */
    killScrollTriggerAnimation() {
        ...
        this.gsap.set(targets, { clearProps: 'transform,opacity,scale' });
        this.scrollTriggers = [];
    }
    /**
     * @function handleScrollEvent
     * Handles the scroll event animation for the page using GSAP (GreenSock Animation Platform) and its ScrollTrigger plugin.
     *
     * This function creates a GSAP timeline controlled by the scroll position, starting and ending based on the position of `_lppElement` in the viewport.
     * The `scrub` option ensures smooth animation following the scroll position, while the `pin` option is set to 'none', so the element isn't fixed during the scroll.
     *
     * If certain elements exist, the function initiates animations for them, including changes in position and opacity, with varying durations and delays.
     *
     * GSAP is a JavaScript library for high-performance animations that can animate any numeric property of any object.
     * ScrollTrigger, a GSAP plugin, enables triggering animations based on scroll position, useful for scroll-driven animations.
     */
    handleScrollEvent() {
        if (this.noAnimation) {
        ...
        const lpp = this.gsap.timeline({
            scrollTrigger: {
                trigger: this._lppElement,
                start: 'top bottom',
        ...
        if (this._heading) {
            lpp.fromTo(this._heading, { y: 100 }, { ease: 'power1.out', y: 0, duration: 1 }, 0.3);
        }
        if (this._description) {
            lpp.fromTo(this._description, { opacity: 0, y: 100 }, { ease: 'power1.out', opacity: 1, y: 0, duration: 0.5 }, 0.5);
        }
        if (this._topBtnGroup) {
            lpp.fromTo(this._topBtnGroup, { opacity: 0 }, { ease: 'sine.out', opacity: 1, duration: 1 }, 1);
        }
        if (window.innerWidth < 1084) {
            this.animateArticle(lpp, this._firstArticle, this._firstImage, 1.5, 0.5, 100);
            this.animateArticle(lpp, this._secondArticle, this._secondImage, 2);
            this.animateArticle(lpp, this._thirdArticle, this._thirdImage, 3);
            this.animateArticle(lpp, this._fourthArticle, this._fourthImage, 4);
        }
        else {
            this.animateArticle(lpp, this._secondArticle, this._secondImage, 1.5);
            this.animateArticle(lpp, this._firstArticle, this._firstImage, 2);
            this.animateArticle(lpp, this._thirdArticle, this._thirdImage, 3);
            this.animateArticle(lpp, this._fourthArticle, this._fourthImage, 4);
        }
        if (lpp.scrollTrigger) {
            this.scrollTriggers.push(lpp.scrollTrigger);
        }
    }
};
LandingPagePathing.styles = landing_page_pathing_styles;
__decorate([
    property_n({ attribute: 'total' })
], LandingPagePathing.prototype, "total", void 0);
__decorate([
    property_n({ attribute: 'no-animation', type: Boolean })
], LandingPagePathing.prototype, "noAnimation", void 0);
__decorate([
    query_e('.landing-page-path')
], LandingPagePathing.prototype, "_lppElement", void 0);
__decorate([
    query_all_r('.lpp-main article')
], LandingPagePathing.prototype, "_lppArticles", void 0);
__decorate([
    query_e('.article-0')
], LandingPagePathing.prototype, "_firstArticle", void 0);
__decorate([
    query_e('.article-1')
], LandingPagePathing.prototype, "_secondArticle", void 0);
__decorate([
    query_e('.article-2')
], LandingPagePathing.prototype, "_thirdArticle", void 0);
__decorate([
    query_e('.article-3')
], LandingPagePathing.prototype, "_fourthArticle", void 0);
__decorate([
    query_assigned_elements_o({ slot: 'lpp-heading' })
], LandingPagePathing.prototype, "_headingSlot", void 0);
__decorate([
    query_assigned_elements_o({ slot: 'lpp-image1' })
], LandingPagePathing.prototype, "_firstImage", void 0);
__decorate([
    query_assigned_elements_o({ slot: 'lpp-image2' })
], LandingPagePathing.prototype, "_secondImage", void 0);
__decorate([
    query_assigned_elements_o({ slot: 'lpp-image3' })
], LandingPagePathing.prototype, "_thirdImage", void 0);
__decorate([
    query_assigned_elements_o({ slot: 'lpp-image4' })
], LandingPagePathing.prototype, "_fourthImage", void 0);
__decorate([
    state_r()
], LandingPagePathing.prototype, "_heading", void 0);
__decorate([
    state_r()
], LandingPagePathing.prototype, "_description", void 0);
__decorate([
    state_r()
], LandingPagePathing.prototype, "_topBtnGroup", void 0);
LandingPagePathing = __decorate([
    custom_element_t('cascade-landing-page-pathing')
], LandingPagePathing);
