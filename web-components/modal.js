...
__webpack_require__.d(__webpack_exports__, {
  CascadeModal: () => (/* binding */ CascadeModal)
});
...
;// CONCATENATED MODULE: ./src/components/modal/modal.styles.ts

/* harmony default export */ const modal_styles = (i `
  dialog {
    min-width: 15rem;
  ...
  :host(.theme-night) dialog,
  :host([color-theme='material-color-dark']) dialog {
    border: none;
  }

  :host(.theme-night) .modal-content,
  :host([color-theme='material-color-dark']) .modal-content {
  ...
  *:focus {
  ...
  dialog[open]::backdrop {
    animation: backdrop-fade-in 0.5s linear forwards;
  }
  ...
  .modal-header {
    ...
    background-color: var(--ds-theme-material-color-background, #fff);
    color: var(--ds-theme-material-color-foreground, #000);
  }
...
/**
 * @description: A simple modal dialog that supports light and dark themes
 * @requires: Trigger element must have a "data-target" attribute set to this modal's id
 * Note: Old browsers may need a polyfill for dialog element (https://github.com/GoogleChrome/dialog-polyfill)
 */
let CascadeModal = class CascadeModal extends WithLocalization {
    constructor() {
        super(...arguments);
        this.colorTheme = 'material-color-light';
    }
    connectedCallback() {
        super.connectedCallback();
    ...
    async handleOpen(event) {
        ...
        if (dialog?.open || !dialog.isConnected)
            return true;
        if (event.target?.dataset.target === '#' + this.id) {
    ...
    disconnectedCallback() {
        super.disconnectedCallback();
        document.removeEventListener('click', this.handleOpen.bind(this));
    }
    async handleClose(event) {
        const target = event.composedPath()?.[0];
        const [dialog, closeBtn] = [await this.dialog, await this.closeBtn];
        if (!target)
            return;
        // target is "dialog" if clicking on the backdrop
    ...
    render() {
        return x `
      <dialog part="base" id="dialog-${this.id}" fade>
        <div part="backdrop" class="modal-content material-backdrop">
          <div part="header" class="modal-header">
            <button
              part="action-close-modal"
              type="button"
              class="close"
              title="${this.msg('closeTooltip')}"
              aria-label="${this.msg('closeAria')}"
              data-dismiss="modal"
            ></button>
...
CascadeModal.dict = {
    closeAria: 'Close dialog window',
    closeTooltip: 'close',
};
CascadeModal.styles = [materialThemes, modal_styles];
__decorate([
    property_n({ attribute: 'color-theme' })
], CascadeModal.prototype, "colorTheme", void 0);
__decorate([
    query_async_r('dialog')
], CascadeModal.prototype, "dialog", void 0);
__decorate([
    query_async_r('.close')
], CascadeModal.prototype, "closeBtn", void 0);
CascadeModal = __decorate([
    defineCustomElement('cascade-modal')
], CascadeModal);
